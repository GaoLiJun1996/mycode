...
bool isShengPiZi(unsigned char* chinese)
{
	assert(chinese!= NULL);
	unsigned long code;
	enc_utf8_to_unicode_one(( unsigned char*)chinese,&code);
	int xia = 19968;
	int shang = 40869;
	std::cout<<std::hex<<xia<<shang<<std::endl;
	auto flag = false;
	if(code<xia || code > shang)
		flag = true;
	return flag;
}
...
int main()
{
	...
	cout<<s<<endl;
	unsigned long  code;
	auto ff = isShengPiZi((unsigned char*)"汉");
	cout<<ff<<endl;
	return 0;
}
